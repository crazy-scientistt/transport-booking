/**
 * EXAMPLE: Updated WelcomePopup Component with Dynamic Ramadan Pricing
 * 
 * This shows how to integrate the new Ramadan detection system
 * Copy relevant sections to client/src/components/WelcomePopup.tsx
 */
...
import { useIsDateRamadan } from '@/contexts/RamadanContext'; // NEW: Import Ramadan hook
import { getPriceForServiceOnDate } from '@/utils/dynamicPricingUtils'; // NEW: Import dynamic pricing
import {
  vehicles,
  Vehicle,
...
export default function WelcomePopup({ open, onClose, onBrowseMore }: WelcomePopupProps) {
  ...
  // NEW: Check if selected date is during Ramadan
  const { isRamadanDate, loading: ramadanLoading } = useIsDateRamadan(selectedDate ?? null);
  ...
  // NEW: Fetch price dynamically based on date
  useEffect(() => {
  ...
  const handleBookNow = () => {
    ...
    toast.success('Added to cart!', {
      description: `${isRamadanDate ? 'Ramadan pricing applied! ' : ''}Your service has been added. You can add more from the main site.`,
    });
          ...
          {step === 'vehicle' ? (
            ...
            >
              ...
              <div className="space-y-4">
                ...
                {/* Price Display with Ramadan Indicator */}
                {selectedPrice !== null && (
                  <div className={`flex justify-between items-center p-4 rounded-lg transition-colors ${
                    isRamadanDate && !ramadanLoading
                    ...
                    <div>
                      ...
                      {isRamadanDate && !ramadanLoading && (
                        <div className="text-xs text-amber-700 dark:text-amber-300 mt-1">
                          ⭐ Ramadan Premium Pricing Applied
                        </div>
                      )}
                    </div>
                ...
                <div className="flex flex-col sm:flex-row gap-3 pt-4">
                  <Button
                    ...
                    disabled={!selectedServiceId || !selectedDate || !selectedTime || selectedPrice === null || priceLoading}
                    className="flex-1 bg-emerald hover:bg-emerald/90 text-white font-semibold py-6"
                  >
                    {priceLoading ? 'Calculating Price...' : 'Add to Cart'}
                  </Button>
                </div>
